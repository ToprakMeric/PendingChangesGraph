...
# Her 1000 değişiklikte renk değişecek şekilde 10 seviye
COLOR_IDS = [
	"green",
	"lime",
	"yellow",
...
COLOR_DEFINITIONS = [
	...
	" id:darkred value:rgb(0.55,0,0)",
        " id:maxred value:rgb(0.33,0,0)"
]
...
def get_color_id(value):
	if value is None or value <= 0:
		return COLOR_IDS[0]
	bucket = min(value // 1000, len(COLOR_IDS) - 1)
	return COLOR_IDS[bucket]
...
def generate_timeline(data_dict, bar_increment, width, period_type="auto"):
	...
	for label in sorted(data_dict.keys()):
		...
		# Etiket saat formatındaysa ("00"-"23") .00 ekle
		if label.isdigit() and len(label) == 2:
			bar_label = f"{label}.00"
		else:
			bar_label = datetime.strptime(label, "%Y-%m-%d").strftime("%m-%d")
...
def generate_timeline_for_day(data_for_day):  # keys: "00", "01", ... "23"
...
def main():
	try:
		...
		data_page = "Vikipedi:Bekleyen değişiklikler grafiği/data"
		timeline_page = "Vikipedi:Bekleyen değişiklikler grafiği"
...
def run_at_exact_hour():
	while True:
		...
		if now.minute == 0:
			...
			time.sleep(61)  # aynı saatte ikinci kez tetiklememek için
		else:
			time.sleep(30)
